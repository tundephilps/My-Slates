...
const user=JSON.parse(sessionStorage.getItem("personalInfos")||"null");
if(!user)return navigate("/PersonalInformation")
  setWorking(true)
console.log(user);
createUserWithEmailAndPassword(auth,user.email,password).then(async (credentials)=>{
const createdUser=credentials.user
const userData={...user,uid:createdUser.uid}
    ...
    <div className="flex flex-col relative items-center justify-between py-8 h-screen w-full text-black">
      ...
      <div>
        ...
        <div className="mt-2 w-full flex flex-col space-y-2">
          ...
          <button onClick={submit} disabled={working} className="bg-[#047aa5] text-white text-sm py-3 rounded-md font-semibold w-full">
